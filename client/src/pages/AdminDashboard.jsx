...
const AdminDashboard = () => {
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-[#111] flex flex-col items-center justify-center p-4 font-outfit text-white" style={{ backgroundImage: 'radial-gradient(circle at top, #2a2a2a, #111)' }}>
        <motion.form 
      ...
      <div className="bg-black/80 backdrop-blur-xl border-b border-white/10 shadow-2xl sticky top-0 z-40">
        <div className="max-w-[1600px] mx-auto px-6 h-24 flex justify-between items-center">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h1 className="font-playfair font-black text-2xl text-gradient-gold tracking-wide">Admin Control</h1>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em] mt-1">System Online</p>
             </div>
          </div>
          
          <div className="flex items-center gap-4">
            <div className="bg-white/5 p-1.5 rounded-2xl border border-white/10 flex mr-2 overflow-x-auto hide-scrollbar">
               {[
                 ...
                 <button 
                  ...
                  className={`px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-hotel-gold text-black shadow-lg shadow-hotel-gold/20' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                 >
                   {tab.label}
                   {tab.id === 'waiter' && waiterRequests.length > 0 && <span className="ml-2 bg-red-500 text-white px-2 py-0.5 rounded-full animate-pulse">{waiterRequests.length}</span>}
                   {tab.id === 'cash' && activeSessions.some(s => s.billRequested) && <span className="ml-2 bg-orange-500 text-white px-2 py-0.5 rounded-full animate-pulse">!</span>}
                 </button>
               ))}
            </div>
            
             {activeTab === 'tables' ? (
               <button onClick={() => setTableCount(prev => prev + 1)} className="bg-white/10 border border-white/10 text-white px-5 py-3 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-white/20 flex items-center gap-2 transition-colors whitespace-nowrap">
               ...
               <button 
                 ...
                 className="bg-white/10 border border-white/10 text-white px-5 py-3 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-white/20 flex items-center gap-2 transition-colors whitespace-nowrap"
               >
                 <Plus size={16} /> New {activeTab === 'items' ? 'Item' : 'Category'}
               </button>
             ) : (
                <button onClick={loadItems} className="bg-white/10 border border-white/10 text-white px-5 py-3 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-white/20 transition-colors whitespace-nowrap">
                  Refresh
                </button>
             )}
          </div>
